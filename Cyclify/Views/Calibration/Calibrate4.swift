...
struct Calibrate4: View {
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                VStack {
                    ...
                    Text("Posture Calibration: Lean Backward")
                        .font(.largeTitle)
                        .fontWeight(.bold)
                    ...
                    NavigationLink(
                        destination:FinishCalibration(userModel: userModel, bluetoothManager: bluetoothManager, calibrationModel: calibrationModel),
                        isActive: $navigateToNext
                    ) {
                        Button(action: {
                            if !isCalibrating && elapsedTime >= 10 {
                                bluetoothManager.sendCalibrationCommand("DONE")
                                navigateToNext = true
    ...
    private func beginCalibration() {
        bluetoothManager.sendCalibrationCommand("CALIBRATE:4")
