...
//  Cyclify
//
//  Created by Carolyn Yang on 3/27/25.
//


import SwiftUI

struct Calibrate1: View {
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                VStack {
                    ...
                    Text("Posture Calibration: Lean Left")
                        .font(.largeTitle)
                        .fontWeight(.bold)
                    ...
                    if !calibrationModel.getReadings(for: .leanLeft).isEmpty {
                        ...
                        ScrollView {
                            VStack(alignment: .leading) {
                                ForEach(Array(calibrationModel.getReadings(for: .leanLeft).enumerated()), id: \.offset) { index, readings in
                    ...
                    NavigationLink(
                        destination: Calibrate2(userModel: userModel, bluetoothManager: bluetoothManager, calibrationModel: calibrationModel),
                    ...
                    do {
                        if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
                           let sensors = json["sensors"] as? [Int] {
                            calibrationModel.addReadings(for: .leanLeft, readings: sensors)
                        }
                    } catch {
    ...
    private func beginCalibration() {
        bluetoothManager.sendCalibrationCommand("CALIBRATE:1")
        print("Sent calibration command 1")
    ...
    private func resetCalibration() {
        isCalibrating = false
        calibrationModel.resetReadings(for: .leanLeft)
        elapsedTime = 0
        timer?.invalidate()
        timer = nil
    }
